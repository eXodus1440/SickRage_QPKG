...
SAB_LINKED=$($CMD_GETCFG General linked_to_sabnzbd -f ${SYS_QPKG_DIR}/.sickrage/config.ini)
...
if [ "$SAB_INSTALLED" == "complete" ] ; then 
  ...
  if [ -f ${SABnzbdPlus_Path}/.sabnzbd/sabnzbd.ini ] ; then
    ...
    $CMD_SETCFG General linked_to_sabnzbd 1 -f ${SYS_QPKG_DIR}/.sickrage/config.ini
  fi
fi
